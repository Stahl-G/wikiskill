...
from wikiskill.spreadsheet.rollout import rollout_case
from wikiskill.wiki import SkillImpactEntry, append_skill_impact

from wikiskill.settings import RESOURCES, DATA_ROOT
PROTOCOL_PATH = RESOURCES / "spreadsheet" / "protocol.json"
DATA = DATA_ROOT / "spreadsheetbench_verified_400"
SPLIT_DIR = RESOURCES / "spreadsheet" / "id_split"
WIKI_TEMPLATE = RESOURCES / "spreadsheet" / "wiki"
PUBLIC_IMPACT = Path("runs") / "skill-impact.md"


def load_split_cases(split: str, limit: int | None = None) -> tuple[SpreadsheetCase, ...]:
    by_uid = {case.uid: case for case in load_cases(DATA)}
    ...
    if missing:
        raise ValueError(f"Spreadsheet split uids missing: {missing[:5]}")
...
def seed_wiki(wiki_dir: Path) -> None:
    ...
    (wiki_dir / "patterns").mkdir(exist_ok=True)
    impact = wiki_dir / "skill-impact.md"
    if not impact.is_file():
        impact.write_text(
            "# Skill Impact Tracker\n\nSpreadsheet WikiSkill loop. Append-only JSON entries follow.\n",
            encoding="utf-8",
        )
...
def hygiene_ledger_note(workdir_root: Path) -> str:
    path = workdir_root / "usage-limit-hygiene.json"
    if not path.is_file():
        return ""
    payload = json.loads(path.read_text(encoding="utf-8"))
    note = str(payload.get("planned_ledger_note") or "").strip()
    if not note:
        return ""
    return f"{note}; "


def mark_hygiene_cleared(workdir_root: Path) -> None:
    path = workdir_root / "usage-limit-hygiene.json"
    if not path.is_file():
        return
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["status"] = "CLEARED"
    payload["cleared_at"] = _now()
    path.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (workdir_root / "notes.md").write_text(
        hygiene_ledger_note(workdir_root).rstrip("; ") + "\n",
        encoding="utf-8",
    )
    ...
    append_skill_impact(
        ...
        SkillImpactEntry(
            ...
            prereg={
                ...
                "pair_id": f"spreadsheet-{arm}-it{iteration}",
            },
    ...
    *,
        ...
        append_skill_impact(
            ...
            SkillImpactEntry(
                **entry,
                ...
                accepted=False,
                notes=f"arm={arm}; no_action; {hygiene_ledger_note(workdir_root)}{result.rationale[:300]}",
            ),
        )
        print(
    ...
    try:
        ...
        cand_rows = run_split(
            ...
            outcomes_path=workdir_root / "val-candidate.jsonl",
            model=model,
            reasoning_effort=reasoning_effort,
            skill_text=skill_md,
            max_workers=max_workers,
    ...
    if accepted:
        (wiki_dir / f"SKILL-it{iteration}.md").write_text(skill_md, encoding="utf-8")
        save_r_best(
    ...
    append_skill_impact(
        ...
        SkillImpactEntry(
            **entry,
            ...
            notes=f"arm={arm}; {hygiene_ledger_note(workdir_root)}{rationale[:300]}",
            accepted=accepted,
        ),
    )
