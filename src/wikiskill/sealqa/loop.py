"""SealQA Eq. 4 loop: train → maintainer → proposer → val accept/rollback."""
...
from wikiskill.benchmarks.sealqa import SealQACase, load_cases
...
from wikiskill.sealqa.rollout import rollout_case
from wikiskill.wiki import SkillImpactEntry, append_skill_impact

from wikiskill.settings import RESOURCES, DATA_ROOT
PROTOCOL_PATH = RESOURCES / "sealqa" / "protocol.json"
DATA = DATA_ROOT / "sealqa" / "seal-0.parquet"
SPLIT_DIR = RESOURCES / "sealqa" / "id_split"
WIKI_TEMPLATE = RESOURCES / "sealqa" / "wiki"
PUBLIC_IMPACT = Path("runs") / "skill-impact.md"


def load_split_cases(split: str, limit: int | None = None) -> tuple[SealQACase, ...]:
    ...
    if missing:
        raise ValueError(f"SealQA split uids missing from pool: {missing[:5]}")
...
def seed_wiki(wiki_dir: Path) -> None:
    ...
    if not impact.is_file():
        impact.write_text(
            "# Skill Impact Tracker\n\nSealQA WikiSkill loop. Append-only JSON entries follow.\n",
            encoding="utf-8",
        )
    ...
    append_skill_impact(
        ...
        SkillImpactEntry(
            ...
            prereg={
                ...
                "pair_id": f"sealqa-{arm}-it{iteration}",
            },
