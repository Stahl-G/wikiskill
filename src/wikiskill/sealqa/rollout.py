"""Drive one SealQA item through ``codex exec`` and score the ``<answer>`` tag."""
...
_WEB_SEARCH_SH = """#!/bin/bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"
# Packaged URL list for this item (fragment identifiers stripped).
# This is not live Google.
cat "$ROOT/urls.txt"
"""
...
def build_prompt(case: SealQACase, skill_text: str) -> str:
    skill_section = ""
    if skill_text.strip():
...
def _stage_workspace(case: SealQACase, workspace: Path) -> None:
    ...
    script.write_text(_WEB_SEARCH_SH, encoding="utf-8")
...
def rollout_case(
    ...
    *,
    ...
    score = 0.0 if returncode not in (0,) or error else score_answer(case.answer, stdout)
    events_text = ""
    ...
    record = {
        "schema_version": "briefloop.sealqa.outcome.v1",
        "case_id": case.uid,
        "uid": case.uid,
        "split": split,
        "model": model,
